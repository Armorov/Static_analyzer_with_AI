...
<body>
    ...
    <script type="text/javascript">
        ...
        function saveNotes() {
            if (selectedNodeId && notesField.value !== undefined) {
...
"""
    # Добавляем актуальные данные вместо заглушек
...
def generate_child_graph(input_data, output_file, vis_network_path='vis-network.min.js'):
    def get_child_nodes_from_tree(func):
        ...
        def traverse(node):
            ...
            func_string = str(node.get('STRING', 'не найдено'))
...
def generate_parrent_graph(input_data, output_file, vis_network_path='vis-network.min.js'):
    def get_parrent_nodes(data):
        ...
        def traverse(func, parent_id=None):
            if not func or 'ID' not in func or 'NAME' not in func:
                return

            func_id = func['ID']
            func_name = func['NAME']
            func_path = func.get('PATH', 'не найдено')
            func_string = str(func.get('STRING', 'не найдено'))
